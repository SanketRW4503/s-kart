...
import { add_data } from '../utility/wishlishSlice';
...
export default function Header() {
  ...
  async function getWishlistData(email) {
    ...
    try {
      ...
      if (json.success == true) {
        for (let i = 0; json.result.wishlist, length !== i; i++) {
          dispatch(add_data(json.result.wishlist))
        }
      }
    } catch (error) {
      console.log(error)
  ...
  useEffect(() => {
    if (loginStatus == true) {
      getWishlistData(userStore?.profile?.email)
    }
  }, [loginStatus])
